...
The layout of the environment is randomically generated, but the structure remains the same.
It has 4 rooms, 3 corridors and 7 doors and can be described by the following image:

!!!!!!!!! ADD ENVIORONMENT IMAGE!!!!!!!!!!!!!!

The difference between a room and a corridor is that a coridor has more than one door connected to itself.
This map is generated by interacting with an ontology defined using the software [Protèjè](https://protege.stanford.edu). 
The robot starts in the 'E' location and waits until the topological map has been correctly initialized and 
defined.
Once the map is completed, the robot moves in a new location and waits some time to simulate a 
survaillance task. Once the location has been checked, the robot visits another location.
When the battery of the robot is low it goes in the charging location, which is the 'E' corridor. 
The battery is charged after a random time chosen inside a pre-defined interval.
When the battery is fully charged the robot starts again his survaillance behavior.
When the robot's battery is not low, the robot moves in the environment according to the following 
survaillance policy:
- The robot should mainly stay on corridors.
